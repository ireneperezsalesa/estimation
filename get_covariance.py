...
# Read annotations from json file (one file for groundtruth and another from the processing method that we want to evaluate, containing predictions from Detectron2)
...
for file in files:
    ...
    json_file.close()
    if file.startswith('gt'): # Groundtruth files
		# Add all "person" instances detected
        for ann in json_data:
            detected_person = False
            if ann == []:
                # No predictions found for this image
                p = numpy.matrix([[0],[0]])
            else:
                for inst in ann:
                    if inst["category_id"] == 0 and detected_person is False: # Person, consider only one prediction in case there is more than one "person" instances
            ...
            if points_gt is None:
                ...
                points_gt = numpy.concatenate((points_gt, p), axis = 1) # Every column of the matrix corresponds to one point

    if file.startswith('modo0'): # Method 0 files
        for ann in json_data:
            detected_person = False
            if ann == []:
                p = numpy.matrix([[0],[0]])
                ...
                for inst in ann:
                    if inst["category_id"] == 0 and detected_person is False:  
                        bbox = inst["bbox"]  
                        x = bbox[0] + bbox[2] / 2
                        y = bbox[1] + bbox[3]
                        p = numpy.matrix([[x], [y]])
            ...
            if points_m0 is None:
                ...
                points_m0 = numpy.concatenate((points_m0, p), axis=1) 

    if file.startswith('modo1'): # Method 1 files
        for ann in json_data:
            detected_person = False
            if ann == []:
            ...
            if points_m1 is None:
                ...
                points_m1 = numpy.concatenate((points_m1, p), axis=1)
...
# Compute world coordinates for the detected image points
# Undistort points
...
# The value of the "noise" (v_pk) in the measurement is the difference between the coordinates predicted by the method and the groundtruth value
